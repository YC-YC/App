...
public class BlueToothMusicInfo implements BaseImp, Fresh {
	...
	private static final String URL_BT_ALL = "http://127.0.0.1:8989/bt?req = all& item=null";
	private static final String URL_BTMUSIC_ALL = "http://127.0.0.1:8989/btmusic?req = all& item=null";
	private static final String POST_URL_BTMUSIC = "http://127.0.0.1:8989/btmusic";

	public BlueToothMusicInfo(Context context) {
		mContext = context;
	...
	private boolean getBtInfo() {
		boolean isChange = false;
		String result = HttpUtil.getRequest(URL_BT_ALL);
		if (result == null)
		{
			return false;
		}
	...
	private boolean getBtMusicInfo() {
		boolean isChange = false;
		String result = HttpUtil.getRequest(URL_BTMUSIC_ALL);
		if (result == null)
		{
			return false;
		}
		...
		try {
			JSONObject msgs = (JSONObject) tokener.nextValue();
			msgs = msgs.getJSONObject("result");
			int state = msgs.getInt("state");
			if (state != T.BtMusic.state)
			{
				isChange = true;
				T.BtMusic.state = state;
			}
	...
	private boolean BluetoothPlay() {
		...
		map.put("key", "play");
		HttpUtil.postRequest(POST_URL_BTMUSIC, map);
		return true;
	}
